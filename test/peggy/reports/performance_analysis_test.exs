defmodule Peggy.Reports.PerformanceAnalysisTest do
  ...
  describe "build/2 dataset" do
    setup do
      ...
      sow =
        Peggy.AnimalsFixtures.animal_fixture(scope,
          ear_tag: "S1",
          stage: "sow",
    ...
    test "build/2 returns periods and three sections", %{scope: scope, sow: sow} do
      Peggy.BreedingFixtures.service_fixture(scope, sow,
        served_at: ~D[2025-03-10],
        service_type: "ai"
      )
    ...
    test "end-to-end: a March service + farrowing + weaning lands in the right columns", %{
      scope: scope,
      sow: sow
    } do
      f =
        Peggy.BreedingFixtures.farrowing_fixture(scope, sow,
          farrowed_at: ~D[2025-03-20],
          born_alive: 11,
          service_type: "ai"
        )

      {:ok, _, _} =
        Peggy.Breeding.record_weaning(scope, f, %{
          weaned_at: ~D[2025-04-14],
          weaned_count: 10,
          batch_tag: "W-E2E"
        })
      ...
      # March
      assert Enum.at(far_count.values, 2) == 1
      assert far_count.acum == 1

      weanings = Enum.find(result.sections, &(&1.key == :weaning))
      pigs = Enum.find(weanings.rows, &(&1.key == :pigs_weaned))
      # April
      assert Enum.at(pigs.values, 3) == 10
    end
  end

  defp build_year(scope),
    do:
      Peggy.Reports.PerformanceAnalysis.build(scope, %{from: ~D[2025-01-01], to: ~D[2025-12-31]})
  ...
  describe "farrowing metrics (pure)" do
    ...
    test "litter composition" do
      ...
      # one < 7 born alive
      assert_in_delta PA.m_pct_small_litter(fs), 50.0, 0.1
      assert_in_delta PA.m_avg(fs, :parity), 4.0, 0.01
      # (13 + 10)/2
      assert_in_delta PA.m_avg_total_born(fs), 11.5, 0.01
      assert_in_delta PA.m_avg(fs, :born_alive), 9.0, 0.01
      # 4/23
      assert_in_delta PA.m_pct_of_total_born(fs, :stillborn), 17.39, 0.1
      ...
      report = %{
        ...
        sections: [
          %{
            ...
            rows: [
              %{key: :total_services, label: "Total services", format: :int, values: [5], acum: 5}
            ]
          }
        ]
      }
