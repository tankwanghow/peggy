...
- **daisyUI 5 is used** in this project (loaded via `app.css`). Build UI with its component classes — `btn`, `badge`, `card`, `modal`, `navbar`, `alert`, `input`, `fieldset`, etc. — and the shared components in `core_components.ex` / `layouts.ex`, rather than hand-rolling equivalents. (This overrides the stock `phx.new` guidance to avoid daisyUI.)
...
LiveViews that require login should **always be placed inside the __existing__ `live_session :require_authenticated_user` block**:
    ...
    scope "/", AppWeb do
      ...
      live_session :require_authenticated_user,
        on_mount: [{PeggyWeb.UserAuth, :require_authenticated}] do
...
LiveViews that can work with or without authentication, **always use the __existing__ `:current_user` scope**, ie:
    ...
    scope "/", MyAppWeb do
      ...
      live_session :current_user,
        on_mount: [{PeggyWeb.UserAuth, :mount_current_scope}] do
